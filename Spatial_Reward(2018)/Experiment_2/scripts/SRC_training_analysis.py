...
# SRC03TR_old was replaced - less than 50% of sacccades went directly to target. SRC10TR_old was replaced - either calibration or performance meant he didn't get much reward feedback (44% of all trials)

# High v Low = 180 degrees apart (different hemifield in both vertical and horizontal axis)
# subjects = ['SRC01TR','SRC02TR','SRC03TR','SRC04TR','SRC09TR','SRC10TR','SRC11TR','SRC12TR','SRC17TR','SRC18TR','SRC19TR','SRC20TR']

# High v Low = 90 degrees apart (different hemifield in vertical only)
# subjects = ['SRC05TR','SRC06TR','SRC07TR','SRC08TR','SRC13TR','SRC14TR','SRC15TR','SRC16TR','SRC21TR','SRC22TR','SRC23TR','SRC24TR']
...
training_gains, training_gains_overall = [[] for i in range(2)]
outlier_percentage = []
subjects_vigor = []
...
for sb in range(len(subjects)):
	...
	print 'correct: ', (len(pre_exclusion_df[pre_exclusion_df['target_correct']==1])*100)/len(pre_exclusion_df)

	# Excluded any participants where target_correct was < 60%, i.e. they didn't get enough opportunity to associate location with reward. SRC10_old removed here (target_correct in only 44% of all trials).
	# Can then include all srts where eyes landed in roi of target even if inaccurate calibration meant they didn't receive reward for that particular trial (receiving reward in general at that location is enough to allow us to take saccade onsets for all trials)
	...
	plt.legend(loc='best')
	os.chdir('/Users/bronagh/Documents/Spatial_Reward_Toolbox/experiment_2/results/figs/learning_curves')
	...
	### SACCADE VIGOR ###
	correct_df = correct_df[(correct_df['CURRENT_SAC_AMPLITUDE'] > 0) & (correct_df['CURRENT_SAC_PEAK_VELOCITY'] > 0)]
	
	# Get rid of outliers in the velocity vs amplitude profile, so that fit is not skewed
	y_div_x = correct_df['CURRENT_SAC_PEAK_VELOCITY']/correct_df['CURRENT_SAC_AMPLITUDE']
	mean_y_div_x = np.mean(y_div_x)
	std_y_div_x = np.std(y_div_x)
	
	trials_processed = correct_df[((correct_df['CURRENT_SAC_PEAK_VELOCITY']/correct_df['CURRENT_SAC_AMPLITUDE'])<(mean_y_div_x+(2.5*std_y_div_x))) & ((correct_df['CURRENT_SAC_PEAK_VELOCITY']/correct_df['CURRENT_SAC_AMPLITUDE'])>(mean_y_div_x-(2.5*std_y_div_x)))]
	...
	# They use hyperbolic in papers, but for smaller saccades this appromixates to a linear curve (check whether these saccades are too large)
	...
	expected_v = []
	for i in range(trials_processed.shape[0]):
		expected_v.append([linear_vel(trials_processed['CURRENT_SAC_AMPLITUDE'].iloc[i], params[0],params[1]), trials_processed['trial_number'].iloc[i]]) 

	expected_vel = pd.DataFrame(expected_v, columns = ['expected_vel','trial_number'])
	
	trials = pd.merge(trials_processed, expected_vel, how = 'outer', on= 'trial_number')
	...
	high_vigor = trials[trials['high_location']==trials['target_position']]
	low_vigor = trials[trials['low_location']==trials['target_position']]
	neut1_vigor = trials[neut1==trials['target_position']]
	neut2_vigor = trials[neut2==trials['target_position']]

	high_vigor_mean = np.mean(high_vigor['saccade_vigor'])
	low_vigor_mean = np.mean(low_vigor['saccade_vigor'])
	neut1_vigor_mean = np.mean(neut1_vigor['saccade_vigor'])
	neut2_vigor_mean = np.mean(neut2_vigor['saccade_vigor'])

	subjects_vigor.append([high_vigor_mean,low_vigor_mean, neut1_vigor_mean,neut2_vigor_mean])

vigor = np.array(subjects_vigor)
a = stats.ttest_rel(vigor[:,0],vigor[:,1])
...
plt.errorbar(x,high_list, yerr = std_list[:3], linestyle = '-', color = 'k', label = 'High reward') 
plt.errorbar(x,low_list, yerr = std_list[3:6], linestyle = '--', color = 'k', label = 'Low reward') 
plt.errorbar(x,nocomb_list, yerr = std_list[6:9], linestyle = '-.', color = 'k', label = 'No reward')
plt.legend(loc='best')
...
ax.yaxis.set_label_coords(-0.1, 0.5) # -.1 on x-axis, .5 = halfway along y-axis
ax.xaxis.set_label_coords(0.5, -0.09) 
plt.errorbar(x,high_list, yerr = stdev_list[:3], linestyle = '-', color = 'k', label = 'High reward') 
plt.errorbar(x,low_list, yerr = stdev_list[3:6], linestyle = '--', color = 'k', label = 'Low reward')  
plt.errorbar(x,nocomb_list, yerr = stdev_list[6:9], linestyle = '-.', color = 'k', label = 'No reward')
plt.legend(loc='best')
plt.xlim(xmin=0.5, xmax = 3.5)
plt.xticks([1,2,3])
os.chdir('/Users/bronagh/Documents/Spatial_Reward_Toolbox/experiment_2/results/figs')
